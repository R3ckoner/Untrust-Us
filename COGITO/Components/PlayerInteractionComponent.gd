...
# Power with which objects are thrown (opposed to being dropped)
@export var throw_power: float = 10

var is_changing_wieldables: bool = false # Used to avoid any input actions while wieldables are being swapped
...
@export var grid_map: GridMap  # Reference to the GridMap node for managing soil tiles


func _ready():
	# Attempt to find the GridMap using an absolute path from the root
	grid_map = get_node("/root/HTerrain/GridMap") as GridMap

	if grid_map == null:
		print("GridMap node not found!")
	else:
		print("GridMap node successfully found: ", grid_map.name)
...
func till_soil_at_position(world_position: Vector3):
	if grid_map:
		...
		if current_item != 1:  # Assuming 1 is the ID for tilled soil in your MeshLibrary
			grid_map.set_cell_item(grid_coords, 1)  # Set the tile to tilled soil
			print("Soil tilled at position: ", grid_coords)
		else:
			print("The soil at this position is already tilled.")

# Example method to get the grid position the player is targeting
func get_target_grid_position() -> Vector3:
	var raycast = $InteractionRaycast  # Assuming InteractionRaycast is a child of the player or camera
	if raycast.is_colliding():
		return raycast.get_collision_point()
	
	# Fallback: return the player's current position if no collision is detected
...
func set_state():
	...
	# Clearing out any instantiated wieldable nodes
	...
	# Clearing out equipped wieldable node and its reference
	if equipped_wieldable_node != null:
		equipped_wieldable_node.queue_free()
	equipped_wieldable_node = null
